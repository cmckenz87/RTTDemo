...
function animate_position(e) {
  if (e.parentElement && e.my_visible) {
    ...
    // fade in
    if (!e.my_parent) {
      e.animate([{ opacity: 0 }, { opacity: 1 }], { duration: 500, easing: "ease" });
    }

    if (dx !== 0 || dy !== 0) {
      let dist = Math.sqrt(dx * dx + dy * dy);
      let time = Math.max(500, Math.min(1000, dist / 2));
      e.animate([{ transform: `translate(${dx}px, ${dy}px)` }, { transform: "translate(0, 0)" }], { duration: time, easing: "ease" });
    }
  }
}
...
function ruler_of_region(r) {
  ...
  let n_russian = 0;
  for (let i = 0; i < 12; ++i) {
    if (view.pieces[i] === r) n_afghan++;
    if (view.pieces[i + 12] === r) n_british++;
    if (view.pieces[i + 24] === r) n_russian++;
  }
  ...
  for (let p = 0; p < view.players.length; ++p) {
    let n_tribes = 0;
    let x = 36 + p * 10;
    for (let i = x; i < x + 10; ++i) if (view.pieces[i] === r) n_tribes++;

    let n_ruling = n_tribes;
    if (view.players[p].loyalty === "Afghan") n_ruling += n_afghan;
    if (view.players[p].loyalty === "British") n_ruling += n_british;
    if (view.players[p].loyalty === "Russian") n_ruling += n_russian;
...
function count_cylinders_in_play(p) {
  let n = 0;
  let x = player_cylinders(p);
  for (let i = x; i < x + 10; ++i) if (view.pieces[i] > 0) ++n;
  return n;
}
...
function is_action(action, card) {
  if (view.actions && view.actions[action] && view.actions[action].includes(card)) return true;
  return false;
}

function is_player_action(p) {
  if (view.actions && view.actions["player_" + p]) return true;
  return false;
}

function is_place_gift_action(i) {
  if (view.actions && view.actions.place_gift && view.actions.place_gift.includes(i)) return true;
  return false;
}

function is_suit_action(suit) {
  if (view.actions && view.actions.suit && view.actions.suit.includes(suit)) return true;
  return false;
}

function is_piece_action(i) {
  if (view.actions && view.actions.piece && view.actions.piece.includes(i)) return true;
  return false;
}

function is_space_action(i) {
  if (view.actions && view.actions.space && view.actions.space.includes(i)) return true;
  return false;
}
...
function on_suit_button() {
  if (is_suit_action("Political")) send_action("suit", "Political");
  else if (is_suit_action("Intelligence")) send_action("suit", "Intelligence");
  else if (is_suit_action("Economic")) send_action("suit", "Economic");
  else if (is_suit_action("Military")) send_action("suit", "Military");
  else scroll_into_view_if_needed(ui.board);
}

let open_toggle = true;
function toggle_open_hands() {
  open_toggle = !open_toggle;
  for (let p = 0; p < view.players.length; ++p) if (p !== player_index[player]) ui.player[p].hand.classList.toggle("minimize", open_toggle);
}
...
function layout_armies(list, xc, yc, maxcol) {
  ...
  let i = 0;
  for (let row = 0; row < nrow; ++row) for (let col = 0; col < ncol && i < list.length; ++col) place_army(row, col - ncol / 2 - (nrow - 1) / 4, list[i++]);
}
...
function layout_region_armies(r, xc, yc, maxcol) {
  let list = [];
  for (let i = 0; i < 36; ++i) if (view.pieces[i] === r) list.push(i);
  layout_armies(list, xc - 4, yc, maxcol);
}

function layout_region_tribes(r, xc, yc, maxcol) {
  let list = [];
  for (let i = 36; i < view.pieces.length; ++i) if (view.pieces[i] === r) list.push(i);
  if (list.length > 0) layout_tribes_radial(list, xc, yc, maxcol);
}
...
function is_card_visible(game, c) {
  for (let row = 0; row < 2; ++row) for (let col = 0; col < 6; ++col) if (game.market_cards[row][col] === c) return true;
  for (let p = 0; p < game.players.length; ++p) {
    let pp = game.players[p];
    if (pp.court.includes(c)) return true;
    if (pp.hand.includes(c)) return true;
    for (let evt in pp.events) if (event_cards[evt] === c) return true;
  }
  return false;
}

function is_card_on_market(game, row, c) {
  for (let col = 0; col < 6; ++col) if (game.market_cards[row][col] === c) return true;
  return false;
}
...
function on_update() {
  ...
  if (old_view) {
    ...
    for (let e of ui.pieces) e.my_visible = 0;
  }

  old_view = deep_copy(view);

  function update_event_cards(node, events) {
    for (let evt in events) node.appendChild(ui.cards[event_cards[evt]]);
  }

  let ruler = [ruler_of_region(Persia), ruler_of_region(Transcaspia), ruler_of_region(Herat), ruler_of_region(Kabul), ruler_of_region(Kandahar), ruler_of_region(Punjab)];

  ui.prompt.innerHTML = view.prompt.replace(/#(\d+)/g, sub_card_name);

  //ui.deck_count.textContent = view.cards[0];
  //for (let i = 0; i < 4; ++i) ui.dom_info[i].classList.toggle("hide", i >= view.cards[1]);

  console.log("bar");
  action_button("button_pass_initiative", "Pass");
  ...
  for (let i = 0; i < 10; ++i) action_button("offer_" + i, i);
  ...
  confirm_action_button("end_turn_pass", "End turn", "Are you sure you want to END TURN while you still have actions?");
  ...
  for (let row = 0; row < 2; ++row) {
    for (let col = 0; col < 6; ++col) {
      ...
      if (ce) ce.classList.remove("card_back");
      let me = ui.market_card[row][col];
      if (me.firstChild !== ce) {
        if (me.firstChild) me.removeChild(me.firstChild);
        if (ce) me.appendChild(ce);
      }
      ...
      if (ce) ce.appendChild(ui.market_coin[row][col]);
      else ui.market_coin[row][col].remove();
    }
  }

  for (let i = 1; i < cards.length; ++i) {
    ui.cards[i].classList.toggle("action", is_action("card", i) || is_action("play_left", i) || is_action("play_right", i));
  }
  ...
  for (let i = 301; i <= 309; ++i) ui.spaces[i].classList.toggle("action", is_space_action(i));
  ...
  for (let p = 0; p < view.players.length; ++p) {
    ...
    while (me.firstChild) me.removeChild(me.firstChild);
    ...
    me = ui.player[p].hand;
    while (me.firstChild) me.removeChild(me.firstChild);
    if (p === player_index[player]) me.classList.remove("minimize");
    for (let i = 0; i < pp.hand.length; ++i) {
      let ce = ui.cards[pp.hand[i]];
      if (p !== player_index[player] && !view.open) ce.classList.add("card_back");
      else ce.classList.remove("card_back");
      me.appendChild(ce);
    }
    ...
    if (view.players[p].prizes === 0) {
      ...
      if (view.players[p].prizes === 1) ui.player[p].prize.textContent = view.players[p].prizes + " prize";
      else ui.player[p].prize.textContent = view.players[p].prizes + " prizes";
    }
    ...
    let a = "";
    if (is_player_action(p)) a = " action";
    ...
    for (let i = 9; i >= 0; --i) {
      let x = 36 + p * 10 + i;
      let s = view.pieces[x];
      if (s === 0 || s === Safe_House) ui.player[p].pool.appendChild(ui.pieces[x]);
      else if (s === Gift2) ui.player[p].gift_2.appendChild(ui.pieces[x]);
      else if (s === Gift4) ui.player[p].gift_4.appendChild(ui.pieces[x]);
      else if (s === Gift6) ui.player[p].gift_6.appendChild(ui.pieces[x]);
      else if (s <= 100) ui.spyrows[s].appendChild(ui.pieces[x]);
      else {
        if (ui.pieces[x].parentElement !== ui.board) ui.board.appendChild(ui.pieces[x]);
      }
  ...
  for (let i = 0; i < 6; ++i)
    if (ruler[i] === -1) ui.rule[i].classList = `rule ${space_names[i + Persia]} hide`;
    else ui.rule[i].classList = `rule ${space_names[i + Persia]} ${player_names[ruler[i]]}`;
...
function build_ui() {
  ...
  for (let c = 1; c < cards.length; ++c) {
    ...
    if (c <= 100) {
      ...
      if (passive_cards.includes(c)) e.classList.add("passive");
      if (n === 3) e.classList.add("three");
    } else {
  ...
  ui.rule = [document.querySelector(`#board .rule.Persia`), document.querySelector(`#board .rule.Transcaspia`), document.querySelector(`#board .rule.Herat`), document.querySelector(`#board .rule.Kabul`), document.querySelector(`#board .rule.Kandahar`), document.querySelector(`#board .rule.Punjab`)];
  ...
  ui.dom_info = [document.getElementById("dom_info_1"), document.getElementById("dom_info_2"), document.getElementById("dom_info_3"), document.getElementById("dom_info_4")];
  ...
  if (player !== "Observer") ui.player[top].hand_size.classList.add("hide");
}
