...
if (WEBHOOKS) console.log("Webhook notifications enabled.");
else console.log("Webhook notifications disabled.");

function LOG_STATS() {
  // Count clients connected to join page events
  let num_joins = 0;
  for (let id in join_clients) num_joins += join_clients[id].length;
  ...
  if (num_games > 0 || num_sockets > 0 || num_joins > 0) console.log(`>>> games=${num_games} sockets=${num_sockets} joins=${num_joins}`);
}
...
function login_cookie(req) {
  ...
  if (c) {
    ...
    if (i >= 0) return parseInt(c.substring(i + COOKIE.length));
  }
  return 0;
}
...
function login_touch(res, sid) {
  if (login_sql_touch.run(sid).changes === 1) res.setHeader("Set-Cookie", make_cookie(sid, 2419200));
}
...
function set_static_headers(res, path) {
  if (path.match(/\.(jpg|png|svg|webp|ico|woff2)$/)) res.setHeader("Cache-Control", "max-age=86400, must-revalidate");
...
function play_url(title_id, game_id, role, mode) {
  if (mode && role) return `/${title_id}/play.html?mode=${mode}&game=${game_id}&role=${encodeURIComponent(role)}`;
  else if (mode) return `/${title_id}/play.html?mode=${mode}&game=${game_id}`;
  else if (role) return `/${title_id}/play.html?game=${game_id}&role=${encodeURIComponent(role)}`;
  else return `/${title_id}/play.html?mode=${mode}`;
}
...
function SLOG(socket, ...msg) {
  ...
  let ws = "----------";
  console.log(time, ip, ws, name, "WS", socket.title_id, socket.game_id, socket.role, ...msg);
}

function human_date(date) {
  if (typeof date === "string") date = julianday_from_epoch(Date.parse(date + "Z"));
  if (typeof date !== "number") return "never";
...
function is_valid_user_name(name) {
  ...
  if (SQL_BLACKLIST_NAME.get(name)) return false;
...
app.use(function (req, res, next) {
  ...
  if (sid) {
    ...
    if (user_id) {
      ...
      if (req.user.is_banned) return res.status(403).send("");
    }
  }
...
function must_be_administrator(req, res, next) {
  if (!req.user || req.user.user_id !== 1) return res.status(401).send("Not authorized");
  return next();
}
...
app.post("/login", function (req, res) {
  ...
  if (!is_valid_email(name_or_mail)) name_or_mail = clean_user_name(name_or_mail);
  let user = SQL_SELECT_LOGIN_BY_NAME.get(name_or_mail);
  if (!user) user = SQL_SELECT_LOGIN_BY_MAIL.get(name_or_mail);
  if (!user || is_forbidden_mail(user.mail) || hash_password(password, user.salt) != user.password) return setTimeout(() => res.render("login.pug", { flash: "Invalid login." }), 1000);
...
app.post("/signup", function (req, res) {
  ...
  if (!is_valid_user_name(name)) return err("Invalid user name!");
  if (SQL_EXISTS_USER_NAME.get(name)) return err("That name is already taken.");
  if (!is_valid_email(mail) || is_forbidden_mail(mail)) return err("Invalid mail address!");
  if (SQL_EXISTS_USER_MAIL.get(mail)) return err("That mail is already taken.");
  if (password.length < 4) return err("Password is too short!");
  if (password.length > 100) return err("Password is too long!");
...
function create_and_mail_verification_token(user) {
  if (!SQL_FIND_TOKEN.get(user.user_id)) mail_verification_token(user, SQL_CREATE_TOKEN.get(user.user_id));
}

app.get("/verify-mail", must_be_logged_in, function (req, res) {
  if (SQL_SELECT_USER_VERIFIED.get(req.user.user_id)) return res.redirect("/profile");
...
app.post("/reset-password", function (req, res) {
  ...
  if (password.length < 4) return err("Password is too short!");
  if (password.length > 100) return err("Password is too long!");
  if (!SQL_VERIFY_TOKEN.get(user.user_id, token)) return err("Invalid or expired token!");
  let salt = crypto.randomBytes(32).toString("hex");
  let hash = hash_password(password, salt);
...
app.post("/change-password", must_be_logged_in, function (req, res) {
  ...
  if (newpass.length < 4) return res.render("change_password.pug", { user: req.user, flash: "Password is too short!" });
  if (newpass.length > 100) return res.render("change_password.pug", { user: req.user, flash: "Password is too long!" });
  let oldhash = hash_password(oldpass, user.salt);
  if (oldhash !== user.password) return res.render("change_password.pug", { user: req.user, flash: "Wrong password!" });
  let salt = crypto.randomBytes(32).toString("hex");
...
app.post("/delete-account", must_be_logged_in, function (req, res) {
  ...
  if (hash !== user.password) return res.render("delete_account.pug", { user: req.user, flash: "Wrong password!" });

  let list = SQL_SELECT_GAME_ROLE_FOR_DELETED_USER.all(req.user.user_id);
  for (let item of list) send_chat_message(item.game_id, null, null, `${user.name} (${item.role}) left the game.`);
...
app.post("/api/webhook/update", must_be_logged_in, function (req, res) {
  ...
  if (webhook) send_webhook(req.user.user_id, webhook, "Test message!", 0);
...
app.post("/change-name", must_be_logged_in, function (req, res) {
  ...
  if (!is_valid_user_name(newname)) return res.render("change_name.pug", { user: req.user, flash: "Invalid user name!" });
  if (SQL_EXISTS_USER_NAME.get(newname)) return res.render("change_name.pug", { user: req.user, flash: "That name is already taken!" });
...
app.post("/change-mail", must_be_logged_in, function (req, res) {
  ...
  if (!is_valid_email(newmail) || is_forbidden_mail(newmail)) return res.render("change_mail.pug", { user: req.user, flash: "Invalid mail address!" });
  if (SQL_EXISTS_USER_MAIL.get(newmail)) return res.render("change_mail.pug", { user: req.user, flash: "That mail address is already taken!" });
...
app.get("/user/:who_name", function (req, res) {
  ...
  if (who) {
    ...
    let relation = 0;
    if (req.user) relation = SQL_SELECT_RELATION.get(req.user.user_id, who.user_id) | 0;
...
app.get("/contacts/remove/:who_name", must_be_logged_in, function (req, res) {
  ...
  if (!who) return res.status(404).send("User not found.");
...
app.get("/inbox", must_be_logged_in, function (req, res) {
  ...
  for (let i = 0; i < messages.length; ++i) messages[i].time = human_date(messages[i].time);
...
function show_forum_page(req, res, page) {
  ...
  let threads;
  if (req.user) threads = FORUM_LIST_THREADS_USER.all(req.user.user_id, FORUM_PAGE_SIZE, FORUM_PAGE_SIZE * (page - 1));
  else threads = FORUM_LIST_THREADS.all(FORUM_PAGE_SIZE, FORUM_PAGE_SIZE * (page - 1));
  for (let thread of threads) thread.mtime = human_date(thread.mtime);
...
function linkify_post(text) {
  ...
  text = text.replace(/https?:\/\/\S+/g, (match) => {
    if (match.endsWith(".jpg") || match.endsWith(".png") || match.endsWith(".svg")) return `<a href="${match}"><img src="${match}"></a>`;
...
app.get("/forum/thread/:thread_id", function (req, res) {
  ...
  if (!thread) return res.status(404).send("Invalid thread ID.");
  ...
  if (req.user) FORUM_MARK_READ.run(req.user.user_id, thread_id);
...
app.get("/admin/delete-post/:post_id", must_be_administrator, function (req, res) {
  ...
  res.send(JSON.stringify(FORUM_DELETE_POST.run(post_id)));
});
...
app.post("/forum/post", must_be_logged_in, function (req, res) {
  ...
  if (subject.length === 0) subject = "Untitled";
...
app.get("/forum/edit/:post_id", must_be_logged_in, function (req, res) {
  ...
  if (!post || post.author_id != req.user.user_id) return res.status(404).send("Invalid post ID.");
...
app.get("/forum/search", must_be_logged_in, function (req, res) {
  ...
  let results = [];
  if (search) results = FORUM_SEARCH.all(search);
...
function parse_game_options(options_json) {
  if (options_json in PARSE_OPTIONS_CACHE) return PARSE_OPTIONS_CACHE[options_json];
...
function option_to_english(k) {
  ...
  if (typeof k === "string") return k.replace(/_/g, " ").replace(/^\w/, (c) => c.toUpperCase());
  return k;
}

function format_options(options_json, options) {
  if (options_json in HUMAN_OPTIONS_CACHE) return HUMAN_OPTIONS_CACHE[options_json];
  let text = Object.entries(options)
    .map(([k, v]) => {
      if (k === "players") return v + " Player";
      if (v === true || v === 1) return option_to_english(k);
...
function get_game_roles(title_id, scenario, options) {
  ...
  if (typeof roles === "function") return roles(scenario, options);
  return roles;
}

function is_game_ready(player_count, players) {
  if (player_count !== players.length) return false;
  for (let p of players) if (p.is_invite) return false;
  return true;
}
...
function unload_module(filename) {
  ...
  if (mod) {
    ...
    for (let child of mod.children) unload_module(child.filename);
  }
}
...
function load_rules(rules_dir, rules_file, title) {
  ...
  for (let setup of title.setups) {
    if (!setup.setup_name) {
      if (title.setups.length > 1 && setup.scenario !== "Standard") setup.setup_name = title.title_name + " - " + setup.scenario;
      else setup.setup_name = title.title_name;
    }
...
function watch_rules(rules_dir, rules_file, title) {
  ...
  if (mod) {
    for (let child of mod.children) watch_list.push(child.filename);
  }
  ...
  for (let file of watch_list) fs.watchFile(file, reload_rules);
}
...
function check_create_game_limit(user) {
  if (user.waiting > LIMIT_WAITING_GAMES) return "You have too many games waiting!";
  if (SQL_COUNT_OPEN_GAMES.get(user.user_id) >= LIMIT_OPEN_GAMES) return "You have too many open games!";
  if (SQL_COUNT_ACTIVE_GAMES.get(user.user_id) >= LIMIT_ACTIVE_GAMES) return "You cannot join any more games!";
  return null;
}

function check_join_game_limit(user) {
  if (user.waiting > LIMIT_WAITING_GAMES + 1) return "You have too many games waiting!";
  if (SQL_COUNT_ACTIVE_GAMES.get(user.user_id) >= LIMIT_ACTIVE_GAMES) return "You cannot join any more games!";
  return null;
}
...
function annotate_game_info(game, user_id, unread) {
  ...
  for (let p of game.players) p.index = roles.indexOf(p.role);
  ...
  for (let p of game.players) {
    if (p.user_id === user_id) {
      your_role = p.role;
      your_count++;
      if (p.is_active && game.is_ready && game.status < 2) game.your_turn = true;
      if (p.is_invite) game.your_turn = true;
    }

    if (p.is_active) time_left = Math.min(time_left, p.time_left);

    let link;
    if (p.is_invite) link = `<a class="is_invite" href="/user/${p.name}">${p.name}?</a>`;
    else if (p.is_active) link = `<a class="is_active" href="/user/${p.name}">${p.name}</a>`;
    else link = `<a href="/user/${p.name}">${p.name}</a>`;

    if (game.player_names) game.player_names += ", " + link;
    else game.player_names = link;

    if (game.result === p.role) game.result = `<a href="/user/${p.name}">${p.name}</a> (${game.result})`;
  }
  ...
  if (game.result && game.result.includes(",")) {
    game.result = game.result
      .split(", ")
      .map((role) => {
        for (let p of game.players) if (p.role === role) return `<a href="/user/${p.name}">${p.name}</a>`;
        return role;
      })
      .join(", ");
  }

  if (game.is_ready && game.status === 1) game.time_left = time_left;

  if (your_count > 0) {
    game.is_yours = true;
    if (your_count === 1) game.your_role = your_role;
  }
...
function annotate_games(list, user_id, unread) {
  for (let game of list) annotate_game_info(game, user_id, unread);
  return list;
}
...
app.get("/games/next", must_be_logged_in, function (req, res) {
  ...
  if (next !== undefined) res.redirect(play_url(next.title_id, next.game_id, next.role));
  else res.redirect(`/games/active`);
});
...
function get_title_page(req, res, title_id) {
  ...
  if (!title) return res.status(404).send("Invalid title.");
  let unread = null;
  if (req.user) unread = SQL_SELECT_UNREAD_CHAT_GAMES.all(req.user.user_id);
...
for (let title of TITLE_LIST) app.get("/" + title.title_id, (req, res) => get_title_page(req, res, title.title_id));
...
function options_json_replacer(key, value) {
  ...
  if (typeof value === "string" && String(parseInt(value)) === value) return parseInt(value);
  return value;
}

function is_random_scenario(title_id, scenario) {
  if (RULES[title_id].is_random_scenario) return RULES[title_id].is_random_scenario(scenario);
  return false;
}
...
app.post("/create/:title_id", must_be_logged_in, function (req, res) {
  ...
  if (limit) return res.send(limit);

  if (!(title_id in RULES)) return res.send("Invalid title.");

  if (is_random_scenario(title_id, scenario)) rand = 1;
...
app.get("/delete/:game_id", must_be_logged_in, function (req, res) {
  ...
  if (info.changes === 0) return res.send("Not authorized to delete that game ID.");
  if (info.changes === 1) update_join_clients_deleted(game_id);
...
function insert_rematch_players(old_game_id, new_game_id, req_user_id, order) {
  ...
  if (players.length !== n) throw new Error("missing players");

  switch (order) {
    default:
    case "swap":
      players.sort((a, b) => roles.indexOf(a.role) - roles.indexOf(b.role));
      for (let i = 0; i < n; ++i) players[i].role = roles[(i + 1) % n];
      break;
    case "keep":
      // do nothing
      break;
    case "shuffle":
      ...
      shuffle(players);
      for (let i = 0; i < n; ++i) players[i].role = roles[i];
      break;
    case "random":
      for (let i = 0; i < n; ++i) players[i].role = "Random " + (i + 1);
      break;
  }

  for (let p of players) SQL_INSERT_PLAYER_ROLE.run(new_game_id, p.role, p.user_id, p.user_id !== req_user_id ? 1 : 0);
}
...
app.get("/rematch/:old_game_id", must_be_logged_in, function (req, res) {
  ...
  if (new_game_id) return res.redirect("/join/" + new_game_id);
...
app.post("/rematch/:old_game_id", must_be_logged_in, function (req, res) {
  ...
  try {
    ...
    if (new_game_id) insert_rematch_players(old_game_id, new_game_id, req.user.user_id, order);
    else new_game_id = SQL_SELECT_REMATCH.get(magic);
    SQL_COMMIT.run();
  } catch (err) {
...
app.get("/join/:game_id", function (req, res) {
  ...
  if (req.user) {
    ...
    if (game.owner_id === req.user.user_id) friends = SQL_SELECT_CONTACT_FRIEND_NAMES.all(req.user.user_id);
    if (req.user.user_id === 1) rewind = SQL_SELECT_REWIND.all(game_id);
  }
...
function do_join(res, game_id, role, user_id, user_name, is_invite) {
  ...
  if (game.is_random && game.status === STATUS_OPEN) {
    ...
    if (!m || Number(m[1]) < 1 || Number(m[1]) > roles.length) return res.status(404).send("Invalid role.");
  } else {
    if (!roles.includes(role)) return res.status(404).send("Invalid role.");
  }
  ...
  if (info.changes === 1) {
    ...
    if (is_invite) res.send("Could not invite.");
    else res.send("Could not join game.");
  }
}
...
app.post("/invite/:game_id/:role/:user", must_be_logged_in, function (req, res) {
  ...
  if (user_id) do_join(res, game_id, role, user_id, null, 1);
...
app.post("/accept/:game_id/:role", must_be_logged_in, function (req, res) {
  ...
  if (info.changes === 1) {
    ...
    // send chat message about player joining a game in progress
    if (game.status > 0) send_chat_message(game_id, null, null, `${req.user.name} joined as ${role}.`);
  } else {
...
app.post("/part/:game_id/:role", must_be_logged_in, function (req, res) {
  ...
  if (game.status > 0) {
    if (user_name !== req.user.name) send_chat_message(game_id, null, null, `${user_name} (${role}) left the game (kicked by ${req.user.name}).`);
    else send_chat_message(game_id, null, null, `${user_name} (${role}) left the game.`);
  }
});
...
app.post("/start/:game_id", must_be_logged_in, function (req, res) {
  let game_id = req.params.game_id | 0;
  let game = SQL_SELECT_GAME.get(game_id);
  if (game.owner_id !== req.user.user_id) return res.send("Not authorized to start that game ID.");
  if (game.status !== STATUS_OPEN) return res.send("The game is already started.");
  if (game.join_count !== game.player_count) return res.send("The game does not have enough players.");
...
function start_game(game) {
  ...
  try {
    ...
    if (game.is_random) assign_random_roles(game, options, SQL_SELECT_PLAYERS.all(game.game_id));
...
app.get("/play/:game_id/:role", function (req, res) {
  ...
  if (!title) return res.status(404).send("Invalid game ID.");
...
app.get("/play/:game_id", function (req, res) {
  ...
  if (role) res.redirect(play_url(title, game_id, role));
  else res.redirect(play_url(title, game_id, "Observer"));
});
...
app.get("/api/replay/:game_id", function (req, res) {
  ...
  let game = SQL_SELECT_GAME.get(game_id);
  if (!game) return res.status(404).send("Invalid game ID.");
  if (game.status < STATUS_FINISHED && (!req.user || req.user.user_id !== 1)) return res.status(401).send("Not authorized to debug.");
...
app.get("/admin/rewind/:game_id/:snap_id", must_be_administrator, function (req, res) {
  ...
  try {
    ...
    if (game_clients[game_id]) for (let other of game_clients[game_id]) send_state(other, snap_state);
...
const SQL_CLONE_2 = [SQL(`insert into players(game_id,role,user_id) select $new_game_id,role,user_id from players where game_id=$old_game_id`), SQL(`insert into game_state(game_id,state) select $new_game_id,state from game_state where game_id=$old_game_id`), SQL(`insert into game_replay(game_id,replay_id,role,action,arguments) select $new_game_id,replay_id,role,action,arguments from game_replay where game_id=$old_game_id`), SQL(`insert into game_snap(game_id,snap_id,replay_id,state) select $new_game_id,snap_id,replay_id,state from game_snap where game_id=$old_game_id`)];
...
app.get("/admin/clone/:game_id", must_be_administrator, function (req, res) {
  ...
  try {
    ...
    if (new_game_id) {
      for (let stmt of SQL_CLONE_2) stmt.run({ old_game_id, new_game_id });
    }
    SQL_COMMIT.run();
  } catch (err) {
    return res.send(err.toString());
...
function elo_ev(a, players) {
  ...
  let sum = 0;
  for (let p of players) sum += 10 ** (p.rating / 400);
...
function update_elo_ratings(game_id) {
  ...
  if (!game.result || game.result === "None") return;

  let players = SQL_SELECT_RATING_PLAYERS.all(game_id);

  let winners = 0;
  for (let p of players) if (is_winner(p.role, game.result)) winners++;

  if (winners === 0) return;

  for (let p of players)
    if (is_winner(p.role, game.result)) p.change = elo_change(p, players, 1 / winners);
    else p.change = elo_change(p, players, 0);

  for (let p of players) SQL_INSERT_RATING.run(game.title_id, p.user_id, p.rating + p.change, p.count + 1, game.mtime);
}
...
function mail_verification_token(user, token) {
  if (mailer) {
    ...
    let body = "Your mail verification token is: " + token + "\n\n" + SITE_URL + "/verify-mail/" + token + "\n";
    console.log("SENT MAIL:", mail_addr(user), subject);
    mailer.sendMail({ from: MAIL_FROM, to: mail_addr(user), subject: subject, text: body }, mail_callback);
  }
}
...
async function send_webhook(user_id, webhook, message, retry = 2) {
  ...
  try {
    ...
    if (res.ok) on_webhook_success(user_id);
    else {
      if (retry > 0) retry_webhook(user_id, webhook, message, retry - 1);
      else on_webhook_error(user_id, res.status + ": " + res.statusText);
    }
  } catch (err) {
    if (retry > 0) retry_webhook(user_id, webhook, message, retry - 1);
    else on_webhook_error(user_id, err.message);
  }
}
...
function should_send_reminder(user, game_id) {
  if (!SQL_SELECT_NOTIFIED.get("+23 hours", game_id, user.user_id)) return true;
  return false;
}
...
function send_game_started_notification_to_offline_users(game_id) {
  let players = SQL_SELECT_PLAYERS.all(game_id);
  for (let p of players) {
    if (!is_player_online(game_id, p.user_id)) send_play_notification(p, game_id, "Started");
    delete_last_notified(p, game_id);
  }
}
...
function send_game_finished_notification_to_offline_users(game_id, result) {
  let players = SQL_SELECT_PLAYERS.all(game_id);
  for (let p of players) {
    if (!is_player_online(game_id, p.user_id)) send_play_notification(p, game_id, "Finished (" + result + ")");
    delete_last_notified(p, game_id);
  }
}
...
function is_player_online(game_id, user_id) {
  if (game_clients[game_id]) for (let other of game_clients[game_id]) if (other.user && other.user.user_id === user_id) return true;
  if (join_clients[game_id]) for (let other of join_clients[game_id]) if (other.user_id === user_id) return true;
  return false;
}
...
function send_state(socket, state) {
  try {
    ...
    if (socket.seen < view.log.length) view.log_start = socket.seen;
    else view.log_start = view.log.length;
    socket.seen = view.log.length;
    view.log = view.log.slice(view.log_start);
    if (state.state === "game_over") view.game_over = 1;
...
function get_game_state(game_id) {
  ...
  if (!game_state) throw new Error("No game with that ID");
  return JSON.parse(game_state);
}

function sync_client_state(game_id) {
  for (let socket of game_clients[game_id]) send_state(socket, get_game_state(socket.game_id));
}

function sync_client_state_for_title(title_id) {
  for (let game_id in game_clients) for (let socket of game_clients[game_id]) if (socket.title_id === title_id) send_state(socket, get_game_state(socket.game_id));
}
...
function put_replay(game_id, role, action, args) {
  if (args !== undefined && args !== null && typeof args !== "number") args = JSON.stringify(args);
...
function put_snap(game_id, replay_id, state) {
  ...
  if (game_clients[game_id]) for (let other of game_clients[game_id]) send_message(other, "snapsize", snap_id);
}
...
function put_game_state(game_id, state, old_active, current_role) {
  ...
  if (state.state === "game_over") {
    ...
    if (state.result && state.result !== "None") update_elo_ratings(game_id);
  }
}
...
function put_new_state(game_id, state, old_active, role, action, args) {
  ...
  try {
    ...
    if (state.active !== old_active) put_snap(game_id, replay_id, state);

    put_game_state(game_id, state, old_active, role);

    update_join_clients_game(game_id);
    if (game_clients[game_id]) for (let other of game_clients[game_id]) send_state(other, state);

    if (state.state === "game_over") send_game_finished_notification_to_offline_users(game_id, state.result);
...
function on_action(socket, action, args, cookie) {
  if (args !== null) SLOG(socket, "ACTION", action, JSON.stringify(args));
  else SLOG(socket, "ACTION", action);
  ...
  try {
    ...
    if (old_active !== "Both") game_cookies[socket.game_id]++;
...
function do_resign(game_id, role, how) {
  ...
  if (game.player_count === 2) {
    for (let r of roles) if (r !== role) result = r;
  } else {
...
function on_restore(socket, state_text) {
  if (!DEBUG) send_message(socket, "error", "Debugging is not enabled on this server.");
  SLOG(socket, "RESTORE");
  try {
    ...
    // resend full log!
    for (let other of game_clients[socket.game_id]) other.seen = 0;
...
function on_save(socket) {
  ...
  try {
    ...
    if (!game_state) return send_message(socket, "error", "No game with that ID.");
...
function on_putnote(socket, note) {
  try {
    ...
    if (note.length > 0) SQL_UPDATE_GAME_NOTE.run(socket.game_id, socket.role, note);
    else SQL_DELETE_GAME_NOTE.run(socket.game_id, socket.role);
  } catch (err) {
    console.log(err);
    return send_message(socket, "error", err.toString());
  }
...
function on_getchat(socket, seen) {
  try {
    ...
    if (chat.length > 0) SLOG(socket, "GETCHAT", seen, chat.length);
    for (let i = 0; i < chat.length; ++i) send_message(socket, "chat", chat[i]);
...
function send_chat_message(game_id, from_id, from_name, message) {
  ...
  for (let p of players) {
    ...
    if (!unread) {
      ...
      if (!is_player_online(game_id, p.user_id)) send_chat_activity_notification(game_id, p);
    }
  }

  if (game_clients[game_id]) {
    for (let other of game_clients[game_id]) if (other.role !== "Observer") send_message(other, "newchat", 1);
  }
}
...
function broadcast_presence(game_id) {
  let presence = [];
  for (let socket of game_clients[game_id]) if (!presence.includes(socket.role)) presence.push(socket.role);
  for (let socket of game_clients[game_id]) send_message(socket, "presence", presence);
}
...
wss.on("connection", (socket, req) => {
  ...
  if (u.pathname !== "/play-socket") return setTimeout(() => socket.close(1000, "Invalid request."), 30000);
  req.query = u.query;
  ...
  let user_id = 0;
  let sid = login_cookie(req);
  if (sid) user_id = login_sql_select.get(sid);
  if (user_id) {
  ...
  try {
    ...
    if (!game || game.title_id !== socket.title_id) return socket.close(1000, "Invalid game ID.");

    let players = (socket.players = SQL_SELECT_PLAYERS_WITH_NAME.all(socket.game_id));

    if (socket.role !== "Observer") {
      if (!socket.user) return socket.close(1000, "You are not logged in!");
      ...
      if (socket.role && socket.role !== "undefined" && socket.role !== "null") {
        ...
        if (!me) return socket.close(1000, "You aren't assigned that role!");
      } else {
    ...
    if (socket.seen === 0) {
      let roles = get_game_roles(game.title_id, game.scenario, parse_game_options(game.options));
      send_message(socket, "players", [socket.role, roles.map((r) => ({ role: r, name: players.find((p) => p.role === r)?.name }))]);
    }
    ...
    socket.on("message", (data) => {
      try {
        ...
        if (socket.role !== "Observer") handle_player_message(socket, cmd, arg);
        else handle_observer_message(socket, cmd, arg);
      } catch (err) {
    ...
    if (snapsize > 0) send_message(socket, "snapsize", snapsize);
