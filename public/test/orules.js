...
cards.forEach((card) => {
  if (card) {
    ...
    if (card.name === "EVENT") card.name = card.if_discarded + " / " + card.if_purchased;
  }
});
...
function find_card_in_market(c) {
  for (let row = 0; row < 2; ++row) for (let col = 0; col < 6; ++col) if (c === game.market_cards[row][col]) return [row, col];
  return null;
}
...
function active_has_russian_influence() {
  return active_has_court_card(70);
}
function active_has_persian_influence() {
  return active_has_court_card(68);
}
function active_has_herat_influence() {
  return active_has_court_card(66);
}
function active_has_claim_of_ancient_lineage() {
  return active_has_court_card(5);
}
function active_has_indian_supplies() {
  return active_has_court_card(51);
}
function active_has_well_connected() {
  return active_has_court_card(56);
}
function active_has_strange_bedfellows() {
  return active_has_court_card(21);
}
function active_has_infrastructure() {
  return active_has_court_card(78);
}
function active_has_civil_service_reforms() {
  return active_has_court_card(24);
}
function active_has_charismatic_courtiers() {
  return active_has_court_card(42);
}
function active_has_blackmail_kandahar() {
  return active_has_court_card(43);
}
function active_has_blackmail_herat() {
  return active_has_court_card(54);
}

function player_has_bodyguards(p) {
  return player_has_court_card(p, 15) || player_has_court_card(p, 83);
}
function player_has_indispensable_advisors(p) {
  return player_has_court_card(p, 1);
}
function player_has_citadel_in_kabul(p) {
  return player_has_court_card(p, 17);
}
function player_has_citadel_in_transcaspia(p) {
  return player_has_court_card(p, 97);
}
function player_has_safe_house(p) {
  return player_has_court_card(p, 41) || player_has_court_card(p, 72);
}

function which_player_has_insurrection() {
  return which_player_has_court_card(3);
}
...
function ruler_of_region(r) {
  ...
  let n_russian = 0;
  for (let i = 0; i < 12; ++i) {
    if (game.pieces[i] === r) n_afghan++;
    if (game.pieces[i + 12] === r) n_british++;
    if (game.pieces[i + 24] === r) n_russian++;
  }
  ...
  for (let p = 0; p < game.players.length; ++p) {
    ...
    for (let i = x; i < x + 10; ++i) if (game.pieces[i] === r) n_tribes++;

    let n_ruling = n_tribes;
    if (game.players[p].loyalty === Afghan) n_ruling += n_afghan;
    if (game.players[p].loyalty === British) n_ruling += n_british;
    if (game.players[p].loyalty === Russian) n_ruling += n_russian;

    if (n_ruling === max_ruling) {
      ruler = -1;
    } else if (n_ruling > max_ruling) {
      max_ruling = n_ruling;
      if (n_tribes > 0) ruler = p;
      else ruler = -1;
    }
  }
...
function player_rules_any_region(p) {
  for (let r = first_region; r <= last_region; ++r) if (player_rules_region(p, r)) return true;
  return false;
}
...
function active_can_betray() {
  ...
  for (let i = x; i < x + 10; ++i) {
    if (game.pieces[i] >= 1 && game.pieces[i] <= 100) {
      let c = game.pieces[i];
      if (cards[c].suit !== Political) return true;
      let p = find_card_in_court(c);
      if (!player_has_bodyguards(p)) return true;
    }
  }
  return false;
}
...
function card_has_no_spies(c) {
  for (let p = 0; p < game.players.length; ++p) {
    let x = player_cylinders(p);
    for (let i = x; i < x + 10; ++i) if (game.pieces[i] === c) return false;
  }
  return true;
}
...
function market_cost(col, c) {
  if (col === 0) return 0;
  if (cards[c].patriot === Russian && active_has_russian_influence()) return 0;
  ...
  if (game.favored === Military) return 2 * col;
  return col;
}

function is_favored_suit(c) {
  if (cards[c].suit === game.favored) return true;
  if (player.events.new_tactics && cards[c].suit === Military) return true;
...
function rightmost_card(row, i) {
  while (i >= 0 && game.market_cards[row][i] === 0) --i;
  return i;
}
...
function remove_all_tribes_and_armies(where) {
  ...
  for (let i = 0; i < 36; ++i) if (game.pieces[i] === where) game.pieces[i] = 0;
  ...
  for (let p = 0; p < game.players.length; ++p) {
    ...
    if (maybe_overthrow) check_region_overthrow(p, where);
  }
}
...
function select_available_cylinder() {
  let x = active_cylinders();
  for (let i = x; i < x + 10; ++i) if (game.pieces[i] === 0) return i;
  return -1;
}

function gen_select_cylinder() {
  let x = active_cylinders();
  for (let i = x; i < x + 10; ++i) if (!game.used_pieces.includes(i)) gen_action("piece", i);
}

function gen_select_spy_to_move() {
  let x = active_cylinders();
  for (let i = x; i < x + 10; ++i) if (game.pieces[i] > 0 && game.pieces[i] <= 100) gen_action("piece", i);
}

function gen_select_tribe_to_move() {
  let x = active_cylinders();
  for (let i = x; i < x + 10; ++i) if (game.pieces[i] >= first_region && game.pieces[i] <= last_region) gen_action("piece", i);
}

function select_available_block() {
  let b = active_coalition_blocks();
  for (let i = b + 11; i >= b; --i) if (game.pieces[i] === 0) return i;
  return -1;
}

function select_afghan_block() {
  for (let i = 0; i < 12; ++i) if (game.pieces[i] === 0) return i;
  return -1;
}

function gen_select_block() {
  let b = active_coalition_blocks();
  for (let i = b; i < b + 12; ++i) if (!game.used_pieces.includes(i)) gen_action("piece", i);
}
...
function gen_select_army_to_move() {
  ...
  for (let i = b; i < b + 12; ++i) {
    let r = game.pieces[i];
    if (r >= first_region && r <= last_region) if (active_can_move_from_region(r)) gen_action("piece", i);
  }
}
...
function discard_court_card(c) {
  ...
  if (pidx !== game.phasing) log(`Discarded #${c} from ${player_names[pidx]} court.`);
  else log(`Discarded #${c} from court.`);
  ...
  if (cards[c].leveraged) {
    if (pidx !== game.phasing) log(`${player_names[pidx]} paid back leverage.`);
    else log(`Paid back leverage.`);

    game.players[pidx].coins -= 2;
  }

  if (cards[c].suit === Political) check_court_overthrow(pidx, cards[c].region);
}
...
function change_loyalty(new_loyalty) {
  ...
  for (let i = x; i < x + 10; ++i) {
    let s = game.pieces[i];
    if (s === Gift2 || s === Gift4 || s === Gift6) game.pieces[i] = 0;
  }
...
function check_leverage() {
  ...
  for (let p = first; p < game.players.length; ++p) if (check_player_leverage(p)) return;
  for (let p = 0; p < first; ++p) if (check_player_leverage(p)) return;
  check_safe_house();
}
...
states.leverage = {
  prompt() {
    if (player.coins < 0) {
      ...
      for (let i = 0; i < player.hand.length; ++i) gen_action("card", player.hand[i]);
      for (let i = 0; i < player.court.length; ++i) gen_action("card", player.court[i]);
    } else {
  ...
  card(c) {
    ...
    if (player.hand.length + player.court.length === 0) player.coins = 0;
  },
  next() {
    push_undo();
    check_leverage();
...
function check_court_overthrow(p, r) {
  ...
  for (let i = 0; i < court.length; ++i) {
    ...
    if (info.suit === Political && info.region === r) ++nc;
  }

  let nt = 0;
  for (let i = x; i < x + 10; ++i) if (game.pieces[i] === r) ++nt;
...
function check_region_overthrow(p, r) {
  ...
  if (nt === 0 && nc > 0) {
    ...
    for (let i = 0; i < court.length; ) {
      let info = cards[court[i]];
      if (info.suit === Political && info.region === r) discard_court_card(court[i]);
      else ++i;
    }
  }
}
...
states.bribe = {
  prompt() {
    ...
    if (player.coins - game.reserve >= game.count) gen_action("pay");
    gen_action("beg");
    if (player.events.courtly_manners) gen_action("courtly_manners");
    if (game.undo.length === 0) gen_action("cancel");
  },
  courtly_manners() {
  ...
  beg() {
    let p = game.bribe;
    if (typeof game.where === "string") log(`Asked ${player_names[p]} to waive the bribe to use #${game.card} to ${game.where}.`);
    else log(`Asked ${player_names[p]} to waive the bribe to play #${game.card}.`);
    game.state = "waive";
    set_active(p);
  },
  refuse() {
...
states.waive = {
  prompt() {
    if (typeof game.where === "string") view.prompt = `${player_names[game.phasing]} asks you to waive the bribe to use #${game.card} to ${game.where}.`;
    else view.prompt = `${player_names[game.phasing]} asks you to waive the bribe to play #${game.card}.`;
    let max_cost = Math.min(game.players[game.phasing].coins - game.reserve, game.count);
    gen_action("waive");
    for (let i = 1; i <= max_cost; ++i) gen_action("offer_" + i);
    gen_action("refuse");
  },
  waive() {
...
function end_bribe() {
  if (typeof game.where === "string") do_card_action_2();
  else do_play_2();
}
...
function set_starting_loyalty(loyalty) {
  ...
  if (game.players[next].loyalty) goto_actions();
  else set_active(next);
}
...
states.actions = {
  prompt() {
    ...
    if (game.actions > 0) {
      for (let row = 0; row < 2; ++row) {
        for (let col = 0; col < 6; ++col) {
          ...
          if (c && market_cost(col, c) <= player.coins && !game.used_cards.includes(c) && c !== PUBLIC_WITHDRAWAL) gen_action("card", c);
        }
      }
    }
    ...
    for (let i = 0; i < player.court.length; ++i) {
      ...
      if (game.used_cards.includes(c)) continue;
      ...
      if (favored || game.actions > 0) {
        ...
        if (usable && favored) bonus = true;
      }
    }

    if (game.actions === 2) view.prompt = `You may take two actions.`;
    else if (game.actions === 1) view.prompt = `You may take one more action.`;
    else view.prompt = `You have no more actions.`;
    if (bonus) view.prompt += ` You may take bonus actions.`;
  },
...
function do_play_2() {
  ...
  if (side) player.court.push(c);
  else player.court.unshift(c);
  goto_play_patriot();
}

function goto_play_patriot() {
  let card = cards[game.card];
  if (card.patriot && card.patriot !== player.loyalty) change_loyalty(card.patriot);
  goto_play_tribes();
}
...
states.place_tribe = {
  ...
  space(s) {
    ...
    game.selected = -1;
    if (--game.count === 0) goto_play_roads();
...
states.place_road = {
  ...
  prompt() {
    if (game.selected < 0) {
      ...
      for (let s of borders[game.where]) gen_action("space", s);
    }
  },
  piece(x) {
    push_undo();
  ...
  space(s) {
    ...
    game.used_pieces.push(game.selected);
    game.selected = -1;
    if (--game.count === 0) goto_play_armies();
    else game.selected = select_available_block();
  },
};
...
states.place_army = {
  ...
  space(s) {
    ...
    game.used_pieces.push(game.selected);
    game.selected = -1;
    if (--game.count === 0) goto_play_spies();
    else game.selected = select_available_block();
  },
};
...
states.place_spy = {
  ...
  card(c) {
    push_undo();
    let p = find_card_in_court(c);
    if (p !== game.active) logi(`Spy to #${c} in ${player_names[p]} court.`);
    else logi(`Spy to #${c}.`);
    game.pieces[game.selected] = c;
    game.used_pieces.push(game.selected);
    game.selected = -1;
    if (--game.count === 0) goto_play_leveraged();
    else game.selected = select_available_cylinder();
  },
};
...
const card_action_table = {
  ...
  Build() {
    ...
    if (player.events.nation_building) game.count *= 2;
    game.selected = select_available_block();
    game.reserve = 0;
    game.state = "build";
  },
...
function do_card_action_2() {
  ...
  if (!is_favored_suit(c)) game.actions--;
...
function can_tax_player(active, p, claim) {
  ...
  for (let i = 0; i < court.length; ++i) {
    let c = court[i];
    if (!okay && player_rules_region(active, cards[c].region)) okay = true;
    if (cards[c].suit === Economic) shelter += cards[c].rank;
  }
...
function do_tax_player(p) {
  ...
  player.coins++;
  if (--game.count === 0) end_action();
}

function active_can_tax() {
  for (let row = 0; row < 2; ++row) for (let col = 0; col < 6; ++col) if (game.market_coins[row][col] > 0) return true;
  let claim = active_has_claim_of_ancient_lineage();
  for (let p = 0; p < game.players.length; ++p) if (p !== game.active && can_tax_player(game.active, p, claim)) return true;
  return false;
}

states.tax = {
  prompt() {
    if (game.count === 1) view.prompt = `Tax \u2014 take up to ${game.count} rupee from market cards or players.`;
    else view.prompt = `Tax \u2014 take up to ${game.count} rupees from market cards or players.`;

    for (let row = 0; row < 2; ++row) {
      for (let col = 0; col < 6; ++col) {
        if (game.market_coins[row][col] > 0) gen_action("card", game.market_cards[row][col]);
      }
    }
...
states.gift = {
  ...
  place_gift(cost) {
    ...
    if (cost === 2) game.pieces[game.selected] = Gift2;
    else if (cost === 4) game.pieces[game.selected] = Gift4;
...
states.build = {
      ...
      if (game.selected < 0) {
        ...
        for (let r = first_region; r <= last_region; ++r) {
          if (active_rules_region(r)) {
            gen_action("space", r);
            for (let s of borders[r]) gen_action("space", s);
          }
        }
      }
    }
  ...
  space(s) {
    ...
    let must_pay = !player.events.nation_building || (game.count & 1) === 0;
    if (must_pay) pay_action_cost(2, game.reserve++);

    if (s <= last_region) logi(`${player.loyalty} army to ${region_names[s]}.`);
    else logi(`${player.loyalty} road to ${border_names[s]}.`);
    game.pieces[game.selected] = s;
    game.used_pieces.push(game.selected);
    game.selected = -1;
    --game.count;
    ...
    if (game.count === 0 || (must_pay && player.coins < 2)) end_build_action();
    else game.selected = select_available_block();
  },
  next() {
    push_undo();
...
function last_court_position() {
  let n = 0;
  for (let p = 0; p < game.players.length; ++p) n += game.players[p].court.length;
  return n - 1;
}
...
function court_card_from_position(y) {
  ...
  for (let p = 0; p < game.players.length; ++p) {
    let court = game.players[p].court;
    if (y < x + court.length) return court[y - x];
    x += court.length;
  }
  return 0;
}
...
function can_army_move_across_border(here, next) {
  ...
  let b = active_coalition_blocks();
  for (let i = b; i < b + 12; ++i) if (game.pieces[i] === border) return true;
  return false;
}

function active_can_move_from_region(here) {
  let supplies = active_has_indian_supplies();
  for (let next of roads[here]) if (supplies || can_army_move_across_border(here, next)) return true;
  return false;
}
...
function active_can_move() {
  ...
  for (let i = b; i < b + 12; ++i) {
    let r = game.pieces[i];
    if (r >= first_region && r <= last_region) if (active_can_move_from_region(r)) return true;
  }
...
states.move = {
    ...
    if (game.selected >= 0) {
      ...
      if (here <= 100) {
        ...
        if (active_has_strange_bedfellows()) {
          ...
          for (let p = 0; p < game.players.length; ++p) {
            let court = game.players[p].court;
            for (let i = 0; i < court.length; ++i) if (cards[court[i]].region === r) gen_action("card", court[i]);
          }
        }
      }
      ...
      // Army or tribe in a region
        ...
        if (game.selected < 36) view.prompt = `Move ${player.loyalty} army from ${region_names[here]}.`;
        else view.prompt = `Move ${player_names[game.active]} tribe from ${region_names[here]}.`;
        let supplies = active_has_indian_supplies();
        for (let next of roads[here]) if (supplies || can_army_move_across_border(here, next)) gen_action("space", next);
      }
    } else {
      if (player.events.nationalism) {
        if (game.count === 1) view.prompt = `Move up to ${game.count} spy, army, or tribe \u2014 select a spy, army, or tribe to move.`;
        else view.prompt = `Move up to ${game.count} spies, armies, and/or tribes \u2014 select a spy, army, or tribe to move.`;
      } else {
        if (game.count === 1) view.prompt = `Move up to ${game.count} spy or army \u2014 select a spy or army to move.`;
        else view.prompt = `Move up to ${game.count} spies and/or armies \u2014 select a spy or army to move.`;
      }
      gen_action("next");
      gen_select_army_to_move();
      gen_select_spy_to_move();
      if (player.events.nationalism) gen_select_tribe_to_move();
    }
  },
  piece(x) {
    push_undo();
  ...
  card(c) {
    ...
    let p = find_card_in_court(c);
    if (pold !== p) logi(`Spy from #${old} in ${player_names[pold]} court to #${c} in ${player_names[p]} court.`);
    else logi(`Spy from #${old} to #${c} in ${player_names[p]} court.`);
    game.pieces[game.selected] = c;
    game.selected = -1;
    if (--game.count === 0) end_action();
  },
    ...
    for (let i = x; i < x + 10; ++i) {
      if (game.pieces[i] > 0 && game.pieces[i] <= 100) {
        ...
        if (cards[c].suit !== Political) {
          ...
          let p = find_card_in_court(c);
          if (!player_has_bodyguards(p)) gen_action("card", c);
        }
      }
    }
  },
...
states.accept_prize = {
  ...
  accept() {
    ...
    if (cards[game.card].prize !== player.loyalty) change_loyalty(cards[game.card].prize);
    player.prizes++;
    end_action();
  },
  refuse() {
...
function gen_battle_blocks(where) {
  if (player.loyalty !== Afghan) for (let i = 0; i < 12; ++i) if (game.pieces[i] === where) gen_action("piece", i);
  if (player.loyalty !== British) for (let i = 12; i < 24; ++i) if (game.pieces[i] === where) gen_action("piece", i);
  if (player.loyalty !== Russian) for (let i = 24; i < 36; ++i) if (game.pieces[i] === where) gen_action("piece", i);
}

function count_active_spies_on_card(where) {
  let n = 0;
  let x = active_cylinders();
  for (let i = x; i < x + 10; ++i) if (game.pieces[i] === where) ++n;
  return n;
}
...
function count_enemy_spies_on_card(where) {
  let n = 0;
  for (let p = 0; p < game.players.length; ++p) {
    if (p === game.active) continue;
    if (player_has_indispensable_advisors(p)) continue;
    let x = player_cylinders(p);
    for (let i = x; i < x + 10; ++i) {
      if (game.pieces[i] === where) ++n;
    }
  }
  return n;
}
...
function count_player_armies_in_region(p, where) {
  ...
  for (let i = b; i < b + 12; ++i) if (game.pieces[i] === where) ++n;
  return n;
}
...
function count_player_cylinders(p, r) {
  let x = player_cylinders(p);
  let n = 0;
  for (let i = x; i < x + 10; ++i) if (game.pieces[i] === r) ++n;
  return n;
}

function count_active_armies_in_region(where) {
  let n = count_player_armies_in_region(game.active, where);
  if (player.events.nationalism) n += count_player_cylinders(game.active, where);
  return n;
}

function count_enemy_blocks_on_border(where) {
  let n = 0;
  if (player.loyalty !== Afghan) for (let i = 0; i < 12; ++i) if (game.pieces[i] === where) ++n;
  if (player.loyalty !== British) for (let i = 12; i < 24; ++i) if (game.pieces[i] === where) ++n;
  if (player.loyalty !== Russian) for (let i = 24; i < 36; ++i) if (game.pieces[i] === where) ++n;
  return n;
}
...
function count_enemy_tribes_and_blocks_in_region(where) {
  let n = 0;
  for (let p = 0; p < game.players.length; ++p) {
    if (game.players[p].loyalty === player.loyalty) continue;
    if (player_has_citadel(p, where)) continue;
    let x = player_cylinders(p);
    for (let i = x; i < x + 10; ++i) if (game.pieces[i] === where) ++n;
  }
  if (player.loyalty !== Afghan) for (let i = 0; i < 12; ++i) if (game.pieces[i] === where) ++n;
  if (player.loyalty !== British) for (let i = 12; i < 24; ++i) if (game.pieces[i] === where) ++n;
  if (player.loyalty !== Russian) for (let i = 24; i < 36; ++i) if (game.pieces[i] === where) ++n;
  for (let b of borders[where]) n += count_enemy_blocks_on_border(b);
  return n;
}
...
function active_can_battle() {
  for (let r = first_region; r <= last_region; ++r) {
    if (is_battle_region(r)) return true;
  }
  for (let p = 0; p < game.players.length; ++p) {
    let court = game.players[p].court;
    for (let i = 0; i < court.length; ++i) if (is_battle_card(court[i])) return true;
  }
  return false;
}

function is_safe_house_in_play() {
  for (let x = 36; x < game.pieces.length; ++x) if (game.pieces[x] === Safe_House) return true;
  return false;
}
...
states.battle = {
  prompt() {
    if (game.where <= 0) {
      ...
      for (let p = 0; p < game.players.length; ++p) {
        let court = game.players[p].court;
        for (let i = 0; i < court.length; ++i) {
          if (is_battle_card(court[i])) gen_action("card", court[i]);
        }
      }
      for (let r = first_region; r <= last_region; ++r) {
        if (is_battle_region(r)) gen_action("space", r);
      }
    } else {
      ...
      if (where >= first_region && where <= last_region) {
        ...
        for (let border of borders[where]) gen_battle_blocks(border);
        for (let p = 0; p < game.players.length; ++p) {
          if (p !== game.active && game.players[p].loyalty !== player.loyalty && !player_has_citadel(p, where)) {
            let x = player_cylinders(p);
            for (let i = x; i < x + 10; ++i) if (game.pieces[i] === where) gen_action("piece", i);
          }
        }
      } else {
        if (game.count > 0) {
  ...
  piece(x) {
    ...
    game.pieces[x] = 0;
    if (x < 36) {
      if (where >= first_region && where <= last_region) logi(`Removed ${piece_owner(x)} army from ${region_names[where]}.`);
      else logi(`Removed ${piece_owner(x)} road from ${border_names[where]}.`);
    } else {
      ...
      if (where <= 100) {
        ...
        if (player_has_safe_house(p)) game.pieces[x] = Safe_House;
      } else {
        logi(`Removed ${piece_owner(x)} tribe from ${region_names[where]}.`);
        check_region_overthrow(p, where);
      }
    }
    if (--game.count === 0 && !is_safe_house_in_play()) end_action();
  },
  next() {
    push_undo();
    end_action();
...
function active_can_blackmail(r) {
  if ((r === Herat && active_has_blackmail_herat()) || (r === Kandahar && active_has_blackmail_kandahar())) {
    for (let p = 0; p < game.players.length; ++p) {
      ...
      for (let i = 0; i < court.length; ++i) {
        let c = court[i];
        if (cards[c].region === r && card_has_no_spies(c)) return true;
      }
    }
  }
  return false;
...
states.blackmail = {
  prompt() {
    ...
    let msg = "";
    if (bmh && bmk) msg = "any Herat and/or Kandahar court card without a spy";
    else if (bmh) msg = "any Herat court card without a spy";
    else msg = "any Kandahar court card without a spy";
    if (game.selected < 0) {
      ...
      for (let p = 0; p < game.players.length; ++p) {
        ...
        for (let i = 0; i < court.length; ++i) {
          let c = court[i];
          if (bmh && cards[c].region === Herat && card_has_no_spies(c)) gen_action("card", c);
          if (bmk && cards[c].region === Kandahar && card_has_no_spies(c)) gen_action("card", c);
        }
      }
    }
    gen_action("pass");
...
function check_safe_house() {
  let first = game.phasing;
  for (let p = first; p < game.players.length; ++p) if (check_player_safe_house(p)) return;
  for (let p = 0; p < first; ++p) if (check_player_safe_house(p)) return;
  if (game.actions >= 0) resume_actions();
  else if (game.where === "riot") goto_discard_events();
  else goto_cleanup_hand();
}
...
states.safe_house = {
  ...
  prompt() {
    ...
    if (player.court.indexOf(SAFE_HOUSE_1) >= 0) gen_action("card", SAFE_HOUSE_1);
    if (player.court.indexOf(SAFE_HOUSE_2) >= 0) gen_action("card", SAFE_HOUSE_2);
    gen_action("pass");
  },
  card(c) {
    push_undo();
...
function count_available_afghan_blocks() {
  let n = 0;
  for (let i = 0; i < 12; ++i) if (game.pieces[i] === 0) n++;
  return n;
}
...
function check_insurrection() {
  ...
  if (prince >= 0) {
    ...
    if (prince === game.active || count_available_afghan_blocks() >= 2) game.state = "insurrection";
    else game.state = "insurrection_pause";
  } else {
    end_dominance_check();
  }
}
...
states.insurrection = {
  ...
  space() {
    ...
    game.selected = -1;
    if (--game.count > 0) game.selected = select_afghan_block();
  },
  next() {
    push_undo();
    end_dominance_check();
...
function player_court_size() {
  ...
  for (let i = 0; i < player.court.length; ++i) {
    let c = player.court[i];
    if (cards[c].suit === Political) stars += cards[c].rank;
  }
  return stars;
}
...
function player_hand_size() {
  ...
  for (let i = 0; i < player.court.length; ++i) {
    let c = player.court[i];
    if (cards[c].suit === Intelligence) stars += cards[c].rank;
  }
  return stars;
}
...
states.discard_events = {
  ...
  prompt() {
    ...
    if (is_event_card(game.market_cards[0][0])) gen_action("card", game.market_cards[0][0]);
    else if (is_event_card(game.market_cards[1][0])) gen_action("card", game.market_cards[1][0]);
  },
  card(c) {
    push_undo();
    if (c === game.market_cards[0][0]) do_discard_event(0, c);
    else do_discard_event(1, c);
  },
};
...
function discard_instability_cards() {
  for (let row = 0; row < 2; ++row) {
    for (let col = 0; col < 6; ++col) {
      let c = game.market_cards[row][col];
      if (is_dominance_check(c)) game.market_cards[row][col] = 0;
    }
  }
}
...
function goto_refill_market() {
  ...
  for (let row = 0; row < 2; ++row) {
    for (let col = 0; col < 6; ++col) {
      let c = game.market_cards[row][col];
      if (is_dominance_check(c)) ++instability;
    }
  }
...
states.instability = {
  prompt() {
    ...
    for (let row = 0; row < 2; ++row) {
      for (let col = 0; col < 6; ++col) {
        let c = game.market_cards[row][col];
        if (is_dominance_check(c)) gen_action("card", c);
      }
    }
  },
  card() {
...
const events_if_discarded = {
  ...
  "Failure to Impress"() {
    ...
    for (let p = 0; p < game.players.length; ++p) game.players[p].prizes = 0;
    goto_discard_events();
  },
  ...
  "Confidence Failure"() {
    ...
    if (player.hand.length === 0) next_confidence_failure();
  },
...
function next_confidence_failure() {
  ...
  for (let i = 0; i < 10; ++i) {
    ...
    if (next === game.phasing) return goto_discard_events();
    if (game.players[next].hand.length > 0) return set_active(next);
  }
...
function check_public_withdrawal() {
  ...
  for (let row = 0; row < 2; ++row) for (let col = 0; col < 6; ++col) if (game.market_cards[row][col] === PUBLIC_WITHDRAWAL) game.market_coins[row][col] = 0;
}

states.rumor = {
  prompt() {
    view.prompt = `Rumor \u2014 choose a player.`;
    for (let p = 0; p < game.players.length; ++p) if (p !== game.active) gen_action("player_" + p);
  },
  player_0() {
    do_rumor(0);
  },
...
states.other_persuasive_methods = {
  ...
  player_0() {
    do_other_persuasive_methods(0);
  },
  player_1() {
    do_other_persuasive_methods(1);
  },
  player_2() {
    do_other_persuasive_methods(2);
  },
  player_3() {
    do_other_persuasive_methods(3);
  },
  player_4() {
    do_other_persuasive_methods(4);
  },
};

function do_other_persuasive_methods(p) {
  if (game.open) push_undo();
  else clear_undo(); // No undo if closed hands.
...
function count_cylinders_in_play(p) {
  let n = 0;
  let x = player_cylinders(p);
  for (let i = x; i < x + 10; ++i) if (game.pieces[i] > 0) ++n;
  return n;
}
...
function count_influence_points(p) {
  ...
  if (!game.events.embarrassment_of_riches) {
    ...
    for (let i = x; i < x + 10; ++i) {
      let s = game.pieces[i];
      if (s === Gift2 || s === Gift4 || s === Gift6) n += gv;
    }
  }

  if (!game.players[p].events.rumor) {
    let court = game.players[p].court;
    for (let i = 0; i < court.length; ++i) if (cards[court[i]].patriot) ++n;
  }
...
function assign_vp(points, score, sorted) {
  ...
  while (points.length > 0 && sorted.length > 0) {
    let n = 0;
    for (let p = 0; p < game.players.length; ++p) if (score[p] === sorted[0]) ++n;
    let v = 0;
    for (let i = 0; i < n; ++i) v += points[i] | 0;
    v = Math.floor(v / n);
...
function count_dominance_cards() {
  let n = 0;
  for (let row = 0; row < 2; ++row) for (let col = 0; col < 6; ++col) if (is_dominance_check(game.market_cards[row][col])) ++n;
  for (let i = 0; i < game.deck.length; ++i) if (is_dominance_check(game.deck[i])) ++n;
  return n;
}

function is_final_dominance_check() {
  for (let row = 0; row < 2; ++row) for (let col = 0; col < 6; ++col) if (is_dominance_check(game.market_cards[row][col])) return false;
  for (let i = 0; i < game.deck.length; ++i) if (is_dominance_check(game.deck[i])) return false;
  return true;
}
...
function do_dominance_check(reason) {
  ...
  for (let i = 0; i < 12; ++i) {
    if (game.pieces[i] > 0) n_afghan++;
    if (game.pieces[i + 12] > 0) n_british++;
    if (game.pieces[i + 24] > 0) n_russian++;
  }
  ...
  if (n_afghan >= n_british + limit && n_afghan >= n_russian + limit) success = Afghan;
  else if (n_british >= n_afghan + limit && n_british >= n_russian + limit) success = British;
  else if (n_russian >= n_british + limit && n_russian >= n_afghan + limit) success = Russian;
  ...
  if (success) {
    ...
    logbr();
    if (final) log(`Final Dominance Check.`);
    let list = [];
    for (let p = 0; p < game.players.length; ++p) {
    ...
    for (let p = 0; p < game.players.length; ++p) {
      ...
      if (score[p] !== 1) list.push(`${score[p]} ${player_names[p]} cylinders`);
      else list.push(`${score[p]} ${player_names[p]} cylinder`);
    }
    list.sort(rank_score);
    log("Ranking:\n" + list.join(",\n") + ".");
    if (final) assign_vp([6, 2], score, score.slice());
    else assign_vp([3, 1], score, score.slice());
  }

  // Check instant victory
  let vps = game.players.map((pp) => pp.vp).sort((a, b) => b - a);
  if (vps[0] >= vps[1] + 4) return goto_pause_game_over();

  if (final) return goto_pause_game_over();

  // Clear the board.
  if (success) {
    for (let i = 0; i < 36; ++i) game.pieces[i] = 0;
  }

  game.events = {};
  for (let p = 0; p < game.players.length; ++p) game.players[p].events = {};
...
function vp_tie(pp) {
  ...
  for (let i = 0; i < court.length; ++i) {
    let c = court[i];
    if (cards[c].suit === Military) stars += cards[c].rank;
  }
...
function compute_victory() {
  ...
  for (let i = 0; i < vps.length; ++i) {
    if (vps[i][0] === vps[0][0]) {
      ...
      if (vps[i][1] === game.active) is_winner = true;
    }
  }
...
function prepare_deck() {
  let court_cards = [];
  for (let i = 1; i <= 100; ++i) court_cards.push(i);

  let event_cards = [];
  for (let i = 105; i <= 116; ++i) event_cards.push(i);

  let piles = [[], [], [], [], [], []];

  shuffle(court_cards);

  for (let i = 0; i < 6; ++i) for (let k = 0; k < 5 + game.players.length; ++k) piles[i].push(court_cards.pop());
  ...
  for (let i = 0; i < 6; ++i) shuffle(piles[i]);
...
exports.setup = function (seed, scenario, options) {
  ...
  if (options.rng) game.rng = options.rng;

  prepare_deck();

  for (let col = 0; col < 6; ++col) for (let row = 0; row < 2; ++row) game.market_cards[row][col] = game.deck.pop();
